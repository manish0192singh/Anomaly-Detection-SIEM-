...
@app.get("/")
def root():
    return {"service": "AI SIEM Server", "status": "running",
...
def download(user_id: str):
    ...
    return FileResponse(path=str(out_path), filename="run_pipeline.py",
...
def upload(payload: UploadPayload):
    now = datetime.now().isoformat()
    conn = get_db()
    conn.execute("UPDATE users SET last_seen=?, machine=? WHERE user_id=?",
                 (now, payload.machine, payload.user_id))
    conn.execute("DELETE FROM logs WHERE user_id=?", (payload.user_id,))
    conn.execute("DELETE FROM anomalies WHERE user_id=?", (payload.user_id,))
    conn.execute("DELETE FROM alerts WHERE user_id=?", (payload.user_id,))
    for log in payload.logs:
        conn.execute("""INSERT INTO logs (user_id,timestamp,source,event_id,
            ...
            hour,is_night,is_weekend,uploaded_at) VALUES
            (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (payload.user_id, log.timestamp, log.source, log.event_id,
             log.event_type, log.computer, log.message, log.username,
             log.ip_address, log.event_type_clean, log.hour,
             log.is_night, log.is_weekend, now))
    for a in payload.anomalies:
        conn.execute("""INSERT INTO anomalies (user_id,timestamp,username,
            ...
            (payload.user_id, a.timestamp, a.username, a.ip_address,
    ...
    for al in payload.alerts:
        conn.execute("""INSERT INTO alerts (user_id,alert_id,timestamp,type,
            ...
            (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (payload.user_id, al.id, al.timestamp, al.type, al.message,
    ...
    return {"status": "ok", "user_id": payload.user_id,
            "logs": len(payload.logs), "anomalies": len(payload.anomalies),
...
def get_stats(user_id: str):
    ...
    logs = conn.execute("SELECT COUNT(*) FROM logs WHERE user_id=?",
                        (user_id,)).fetchone()[0]
    anom = conn.execute("SELECT COUNT(*) FROM anomalies WHERE user_id=? AND anomaly=-1",
                        (user_id,)).fetchone()[0]
    alrts = conn.execute("SELECT COUNT(*) FROM alerts WHERE user_id=?",
                         (user_id,)).fetchone()[0]
    crit = conn.execute("SELECT COUNT(*) FROM alerts WHERE user_id=? AND severity='Critical'",
                        (user_id,)).fetchone()[0]
    user = conn.execute("SELECT last_seen, machine FROM users WHERE user_id=?",
                        (user_id,)).fetchone()
    conn.close()
    return {"user_id": user_id, "total_logs": logs, "anomalies": anom,
            "alerts": alrts, "critical": crit,
...
def view(user_id: str):
    ...
    return HTMLResponse(
        f"<html><head><meta http-equiv='refresh' content='0; url={url}'>"
